...
const CARDTEMPLATE = Deno.readTextFileSync("./src/card_template.html");
...
const generateReplacementData = (pokemon, type) => {
  return {
    ...
    "${speed}": pokemon.speed,
    "_color_": type ? TYPECOLOR[type] : TYPECOLOR[pokemon.types[0]],
  };
};

const generateCards = (pokemons, type) => {
  const cards = [];

  pokemons.forEach((pokemon) => {
    const replacementData = generateReplacementData(pokemon, type);
...
const generateIndexPage = (pokemons, navigations) => {
  ...
  Deno.writeTextFileSync("./public/index.html", page);
};

const generateTypesPages = (types, navigations) => {
  for (const type in types) {
    const cards = generateCards(types[type], type);
    const style = listStyleTemplate(type);
    const page = pageTemplate(cards, navigations, style);

    Deno.writeTextFileSync(`./public/${type}.html`, page);
  }
};
